...
export default function OrderContentCreation() {
    ...
    try {
      ...
      if (orderRes.data?.code === 200 && orderRes.data?.data) {
        ...
        setOrderInfo({
          ...
          expectedQuantity: data.expectedQuantity || data.expected_quantity || 1
        })
      }
      ...
      if (avatarsRes.data?.code === 200) {
        ...
        if (avatarList.length > 0) {
          ...
          // 自动开始生成
          setTimeout(() => {
            startGeneration(avatarList[0].id)
          }, 500)
        } else {
          showToast({ title: '没有可用的分身', icon: 'none' })
        }
      } else {
        showToast({ title: '获取分身列表失败', icon: 'none' })
      }
    } catch (err) {
  ...
  const startGeneration = async (avatarIdToUse?: string) => {
    const avatarId = avatarIdToUse || selectedAvatarId
    if (!avatarId) {
      showToast({ title: '没有可用的分身', icon: 'none' })
      return
    }
    ...
    try {
      ...
      const res = await Network.request({
        ...
        data: {
          ...
          contentQuantity: orderInfo?.expectedQuantity || 1
        }
      })
    ...
    return (
      <View className="content-creation-page">
        <View className="loading-container">
          ...
          <Text className="block loading-text">正在加载订单信息...</Text>
        </View>
      </View>
    )
  }
      ...
      <ScrollView scrollY className="content-scroll">
        ...
        {orderInfo && (
          <View className="order-info-card">
            ...
            {orderInfo.expectedQuantity && (
              <View className="info-item">
                <Text className="info-label">生成数量</Text>
                <Text className="info-value">{orderInfo.expectedQuantity}</Text>
              </View>
            )}
          </View>
        )}
        ...
        {/* 自动选中的分身 */}
        {!processingData && avatars.length > 0 && selectedAvatarId && (
          <View className="avatar-select-section">
            <Text className="section-title">执行分身的</Text>
            <View className="avatar-list">
              {avatars.filter(a => a.id === selectedAvatarId).map(avatar => (
                <View key={avatar.id} className="avatar-item selected">
            ...
            <View className="content-preview">
              ...
              {processingData.generatedContent?.images && processingData.generatedContent.images.length > 0 && (
                <View className="images-preview">
                  <Text className="section-title">生成的图片 ({processingData.generatedContent.images.length}张)</Text>
                    ...
                    <View className="images-container">
                      {processingData.generatedContent.images.map((img, idx) => (
                        <View key={idx} className="image-item">
                          <Text className="image-index">第{idx + 1}张</Text>
                          <Image
              ...
              <View className="text-content">
                <Text className="section-title">生成的文案</Text>
