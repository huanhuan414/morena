...
目标：避免改动把代码改乱，让每次修改都具备“范围可控、可回滚、可验证”。
...
- 必须显式列出“允许修改的文件/函数”，禁止越界改动无关文件。
- 禁止顺手行为：不要重排 import、不要批量替换写法、不要重命名导出、不要做与目标无关的“顺手重构”。
- 只对指定函数/片段做最小 diff（局部 patch 优先）。
...
要求：
1) 先给出改动计划（改哪些函数/为什么）
2) 我确认后再给出最小 diff 的修改
3) 改动完成后给出自检点：pnpm validate 是否通过、diff 是否有无关噪音
```
