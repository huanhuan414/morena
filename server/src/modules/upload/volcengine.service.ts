...
export class VolcengineService {
  ...
  async uploadVideo(file: Express.Multer.File): Promise<{ url: string }> {
    ...
    try {
      const serviceId = process.env.VOLCENGINE_VIDEO_SERVICE_ID || '';
      this.logger.log(`[VolcengineService] 视频服务ID: '${serviceId}'`);
      ...
      // 根据veImageX文档，需要使用服务配置的访问域名
      // 从环境变量获取自定义域名，如果没有配置则使用默认域名
      const customDomain = process.env.VOLCENGINE_VIDEO_DOMAIN || process.env.VOLCENGINE_IMAGE_DOMAIN || 'tos-cn-beijing.ivolces.com';
      this.logger.log(`[VolcengineService] 使用域名: ${customDomain}`);
  ...
  async uploadAudio(file: Express.Multer.File): Promise<{ url: string }> {
    this.logger.log(`[VolcengineService] 上传音频功能暂未实现: ${file.originalname}`);
