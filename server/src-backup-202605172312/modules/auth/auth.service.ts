...
export class AuthService {
   ...
   * 验证邀请码是否有效
   */
  async verifyInviteCode(code: string): Promise<{ valid: boolean; inviterNickname?: string; message?: string }> {
    if (!code || !code.trim()) {
      return { valid: false, message: '请输入邀请码' }
    }

    const db = getMySQLClient()
    const inviterResult = await db.query('users', { referral_code: code.trim().toUpperCase() })
    const inviter = Array.isArray(inviterResult) ? inviterResult[0] : (inviterResult as any)?.data?.[0]
    ...
    return {
      valid: true,
      inviterNickname: inviter.nickname || inviter.phone || '邀请人',
      message: '邀请码有效',
    }
  }
  ...
  /**
   * 授权快捷登录（通过邀请码+手机号）
   * 邀请码必填且必须有效，手机号用于标识用户
   */
  async authorizeLogin(inviteCode: string, phone: string): Promise<{
    user: any
    isNewUser: boolean
    token: string
    referralReward?: number
    ...
    if (!phone || !/^1[3-9]\d{9}$/.test(phone)) {
      throw new BadRequestException('请选择有效的手机号')
    }

    const db = getMySQLClient()

    const inviterResult = await db.query('users', { referral_code: inviteCode.trim().toUpperCase() })
    const inviter = Array.isArray(inviterResult) ? inviterResult[0] : (inviterResult as any)?.data?.[0]
    ...
    if (existingUser) {
      if (existingUser.id === inviter.id) {
        throw new BadRequestException('不能使用自己的邀请码登录')
      }
      return {
        user: existingUser,
        isNewUser: false,
    ...
    let referralReward = 0
    try {
      const referralResult = await this.processReferral(newUser.id, inviteCode.trim().toUpperCase())
      referralReward = referralResult.reward
    } catch (error: any) {
      console.error('[AuthService] 授权登录处理邀请码失败:', error.message)
    }
