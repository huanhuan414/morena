...
- ✅ 已落地（代码已合入本地，pnpm validate 通过）
  ...
  - 订单取消/删除联动治理：cancelOrder 统一写入 cancelled（用户主动取消不再写 auto_cancelled），同步失效 pending 派单（order\_dispatch\_requests→expired）、取消进行态内容请求（content\_generation\_requests→cancelled），并补一条 order\_events(cancel)；deleteOrder 增加 earnings 存在性校验（存在收益则禁止物理删除）。[order.service.ts](../server/src/modules/order/order.service.ts)
...
- ✅ 已提供（需在远程 MySQL 执行一次）
  - 增量补齐脚本（幂等）：补齐 content\_generation\_requests 与 notifications 的缺失列，并将 notifications.data 回填到 metadata。[2026-05-17_hotfix_content_notification.sql](../server/src/storage/database/schema/migrations/2026-05-17_hotfix_content_notification.sql)
