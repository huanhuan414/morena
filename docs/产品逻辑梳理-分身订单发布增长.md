...
- 验收：
  - 断网/DB 异常时不会出现“创建成功但刷新消失”；多端登录仍可看到已创建分 身。
...
-  修改方案：
  - 前端：
    ...
    - avatar-create 成功后写 onboarding\_new\_avatar\_id 并 switchTab 到 mind-chat；mind-chat 在 useDidShow 中消费并清理 Storage，并自动滚动定位/高亮新分身。
- 验收：
...
- 取消/删除联动复杂：已按“状态机约束 + 多表联动 + 财务可追溯”修复。取消订单会同步失效 pending 派单、取消进行中的内容请求并写入订单事件；删除订单增加财务保护（存在 earnings 记录则禁止删除），避免对账口径漂移。
...
- ✅ 已落地（代码已合入本地，pnpm validate 通过）
  ...
  - 订单取消/删除联动治理：cancelOrder 统一写入 cancelled（用户主动取消不再写 auto\_cancelled），同步失效 pending 派单（order\_dispatch\_requests→expired）、取消进行态内容请求（content\_generation\_requests→cancelled），并补一条 order\_events(cancel)；deleteOrder 增加 earnings 存在性校验（存在收益则禁止物理删除）。[order.service.ts](../server/src/modules/order/order.service.ts)
  - 通知写库与线上表结构兼容：通知写入/回灌按 notifications 实际列集动态落库，兼容仅有 data 列的旧表，同时兼容 metadata/updated\_at 新列；读取侧 metadata 自动从 metadata/data 解包。[notification.service.ts](../server/src/modules/notification/notification.service.ts)
  - 发布凭证/验证接口与线上表结构兼容：content\_generation\_requests 更新按实际列集动态生成 SET 子句，避免缺列导致 500；并提供标准增量迁移脚本补齐 publish\_url/publish\_screenshot/verification\_status/verified\_at。[content-generation.controller.ts](../server/src/modules/content-generation/content-generation.controller.ts)
- ✅ 已提供（需在远程 MySQL 执行一次）
  - 增量补齐脚本（幂等）：补齐 content\_generation\_requests 与 notifications 的缺失列，并将 notifications.data 回填到 metadata。[2026-05-17\_hotfix\_content\_notification.sql](../server/src/storage/database/schema/migrations/2026-05-17_hotfix_content_notification.sql)
